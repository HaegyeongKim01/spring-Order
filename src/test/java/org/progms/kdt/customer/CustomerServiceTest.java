...
class CustomerServiceTest {
    ...
    @Configuration
    @EnableTransactionManagement  //AOP 관련, advicer 등이 다 적용되게 해줌   //이 어노테이션 삭제 시 정상 동작 X
    static class Config {
    ...
    void testCreateCustomers() {
        var customers = List.of(
                new Customer(UUID.randomUUID(), "a", "a@gmail.com", LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS)),  //woindow의 경우 truncated 함수 사용해야 오류 X
                new Customer(UUID.randomUUID(), "b", "b@gmail.com", LocalDateTime.now().truncatedTo(ChronoUnit.MILLIS))
        );

        customerService.cerateCustomers(customers);
    ...
    @Test
    @DisplayName("다건 추가 실패시 전체 트렌젝션이 롤백되어야한다.")
